...
export async function downloadPacientesCargaMasivaPlantillaWithApi(
  token: string
): Promise<{ blob: Blob; filename: string }> {
  const url = `${getApiBaseUrl()}/api/v1/carga-masiva/pacientes/plantilla`;
  ...
  const filename =
    ...
    "pacientes_carga_masiva.xlsx";

  return { blob, filename };
}

export const CARGA_MASIVA_PACIENTES_MAX_FILE_BYTES = 5 * 1024 * 1024;
export const CARGA_MASIVA_PACIENTES_MAX_FILAS = 1000;

export type CargaMasivaPacientesErrorFilaDto = {
  fila: number;
  campo?: string;
  mensaje: string;
};

export type CargaMasivaPacientesResultDto = {
  totalFilas: number;
  creados: number;
  errores: CargaMasivaPacientesErrorFilaDto[];
};

export function formatCargaMasivaPacienteError(item: CargaMasivaPacientesErrorFilaDto): string {
  const campo = item.campo?.trim();
  return campo ? `${campo}: ${item.mensaje}` : item.mensaje;
}

export function validatePacientesCargaMasivaFile(file: File): string | null {
  ...
  if (file.size > CARGA_MASIVA_PACIENTES_MAX_FILE_BYTES) {
...
function normalizeCargaMasivaResult(raw: Record<string, unknown>): CargaMasivaPacientesResultDto {
  ...
  const errores = Array.isArray(erroresRaw)
    ? erroresRaw
        ...
        .filter((e): e is CargaMasivaPacientesErrorFilaDto => e != null)
    : [];
...
export async function uploadPacientesCargaMasivaWithApi(
  token: string,
  file: File
): Promise<CargaMasivaPacientesResultDto> {
  const url = `${getApiBaseUrl()}/api/v1/carga-masiva/pacientes`;
  ...
  let payload: ApiResponse<CargaMasivaPacientesResultDto> | ApiFailure | null = null;
  try {
    payload = (await response.json()) as ApiResponse<CargaMasivaPacientesResultDto> | ApiFailure;
  } catch {
