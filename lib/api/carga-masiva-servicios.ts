...
export async function downloadServiciosCargaMasivaPlantillaWithApi(
  token: string
): Promise<{ blob: Blob; filename: string }> {
  const url = `${getApiBaseUrl()}/api/v1/carga-masiva/servicios/plantilla`;
  ...
  const filename =
    ...
    "servicios_carga_masiva.xlsx";

  return { blob, filename };
}

export const CARGA_MASIVA_SERVICIOS_MAX_FILE_BYTES = 5 * 1024 * 1024;
export const CARGA_MASIVA_SERVICIOS_MAX_FILAS = 1000;

export type CargaMasivaServiciosErrorFilaDto = {
  fila: number;
  campo?: string;
  mensaje: string;
};

export type CargaMasivaServiciosResultDto = {
  totalFilas: number;
  creados: number;
  errores: CargaMasivaServiciosErrorFilaDto[];
};

export function formatCargaMasivaServicioError(item: CargaMasivaServiciosErrorFilaDto): string {
  const campo = item.campo?.trim();
  return campo ? `${campo}: ${item.mensaje}` : item.mensaje;
}

export function validateServiciosCargaMasivaFile(file: File): string | null {
  ...
  if (file.size > CARGA_MASIVA_SERVICIOS_MAX_FILE_BYTES) {
...
function normalizeCargaMasivaResult(raw: Record<string, unknown>): CargaMasivaServiciosResultDto {
  ...
  const errores = Array.isArray(erroresRaw)
    ? erroresRaw
        ...
        .filter((e): e is CargaMasivaServiciosErrorFilaDto => e != null)
    : [];
...
export async function uploadServiciosCargaMasivaWithApi(
  token: string,
  file: File
): Promise<CargaMasivaServiciosResultDto> {
  const url = `${getApiBaseUrl()}/api/v1/carga-masiva/servicios`;
  ...
  let payload: ApiResponse<CargaMasivaServiciosResultDto> | ApiFailure | null = null;
  try {
    payload = (await response.json()) as ApiResponse<CargaMasivaServiciosResultDto> | ApiFailure;
  } catch {
