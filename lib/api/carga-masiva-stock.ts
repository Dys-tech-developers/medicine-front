...
export async function downloadStockCargaMasivaPlantillaWithApi(
  token: string
): Promise<{ blob: Blob; filename: string }> {
  const url = `${getApiBaseUrl()}/api/v1/carga-masiva/stock/plantilla`;
  ...
  const filename =
    ...
    "stock_carga_masiva.xlsx";

  return { blob, filename };
}

export const CARGA_MASIVA_STOCK_MAX_FILE_BYTES = 5 * 1024 * 1024;
export const CARGA_MASIVA_STOCK_MAX_FILAS = 1000;

export type CargaMasivaStockErrorFilaDto = {
  fila: number;
  campo?: string;
  mensaje: string;
};

export type CargaMasivaStockResultDto = {
  totalFilas: number;
  creados: number;
  errores: CargaMasivaStockErrorFilaDto[];
};

export function formatCargaMasivaStockError(item: CargaMasivaStockErrorFilaDto): string {
  const campo = item.campo?.trim();
  return campo ? `${campo}: ${item.mensaje}` : item.mensaje;
}

export function validateStockCargaMasivaFile(file: File): string | null {
  ...
  if (file.size > CARGA_MASIVA_STOCK_MAX_FILE_BYTES) {
...
function normalizeCargaMasivaResult(raw: Record<string, unknown>): CargaMasivaStockResultDto {
  ...
  const errores = Array.isArray(erroresRaw)
    ? erroresRaw
        ...
        .filter((e): e is CargaMasivaStockErrorFilaDto => e != null)
    : [];
...
export async function uploadStockCargaMasivaWithApi(
  token: string,
  file: File
): Promise<CargaMasivaStockResultDto> {
  const url = `${getApiBaseUrl()}/api/v1/carga-masiva/stock`;
  ...
  let payload: ApiResponse<CargaMasivaStockResultDto> | ApiFailure | null = null;
  try {
    payload = (await response.json()) as ApiResponse<CargaMasivaStockResultDto> | ApiFailure;
  } catch {
