...
import {
  CARGA_MASIVA_PACIENTES_MAX_FILAS,
  downloadPacientesCargaMasivaPlantillaWithApi,
  formatCargaMasivaPacienteError,
  triggerBrowserFileDownload,
  uploadPacientesCargaMasivaWithApi,
  validatePacientesCargaMasivaFile,
...
type Props = {
  ...
  onClose: () => void;
  onSuccess?: (result: CargaMasivaPacientesResultDto) => void;
};
...
export function PacienteCargaMasivaDialog({
  open,
  accessToken,
  onClose,
  onSuccess,
  ...
  const [result, setResult] = useState<CargaMasivaPacientesResultDto | null>(null);
  ...
  const pickFile = (next: File | null) => {
    ...
    const validationError = validatePacientesCargaMasivaFile(next);
    if (validationError) {
  ...
  const handleDownloadPlantilla = async () => {
    ...
    setError("");
    try {
      const { blob, filename } = await downloadPacientesCargaMasivaPlantillaWithApi(accessToken);
  ...
  const handleUpload = async () => {
    ...
    setError("");
    try {
      const data = await uploadPacientesCargaMasivaWithApi(accessToken, file);
      setResult(data);
      onSuccess?.(data);
    } catch (err) {
      const msg =
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="paciente-carga-masiva-title"
    >
      <button
        type="button"
        ...
        <div className="flex items-center justify-between border-b border-medical-border bg-medical-primary px-5 py-4">
          <div className="flex min-w-0 items-center gap-2 text-white">
            ...
            <div className="min-w-0">
              <h2 id="paciente-carga-masiva-title" className="text-base font-semibold">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-5 sm:px-6">
          <div className="rounded-xl border border-medical-border bg-medical-surface/50 px-4 py-3">
            <p className="text-sm text-medical-text">
              Usá la planilla descargada desde el sistema (hoja «Pacientes»). Máximo{" "}
              {CARGA_MASIVA_PACIENTES_MAX_FILAS} filas y 5 MB por archivo.
            </p>
            <Button
              type="button"
              variant="outline"
          ...
          <div>
            ...
            >
              ...
              <p className="mt-1 text-xs text-medical-mutedText">
                Solo .xlsx · hasta 5 MB · máx. {CARGA_MASIVA_PACIENTES_MAX_FILAS} filas con datos
              </p>
              {file ? (
                  ...
                  <ul className="mt-2 max-h-40 space-y-1 overflow-y-auto text-xs text-medical-danger">
                    {result.errores.map((item) => (
                      <li key={`${item.fila}-${item.campo ?? ""}-${item.mensaje}`}>
                        ...
                        {formatCargaMasivaPacienteError(item)}
                      </li>
                    ))}
                  </ul>
                </>
