...
import {
  CARGA_MASIVA_SERVICIOS_MAX_FILAS,
  downloadServiciosCargaMasivaPlantillaWithApi,
  formatCargaMasivaServicioError,
  triggerBrowserFileDownload,
  uploadServiciosCargaMasivaWithApi,
  validateServiciosCargaMasivaFile,
...
type Props = {
  ...
  onClose: () => void;
  onSuccess?: (result: CargaMasivaServiciosResultDto) => void;
};
...
export function ServicioCargaMasivaDialog({
  open,
  accessToken,
  onClose,
  onSuccess,
  ...
  const [result, setResult] = useState<CargaMasivaServiciosResultDto | null>(null);
  ...
  const pickFile = (next: File | null) => {
    ...
    const validationError = validateServiciosCargaMasivaFile(next);
    if (validationError) {
  ...
  const handleDownloadPlantilla = async () => {
    ...
    setError("");
    try {
      const { blob, filename } = await downloadServiciosCargaMasivaPlantillaWithApi(accessToken);
  ...
  const handleUpload = async () => {
    ...
    setError("");
    try {
      const data = await uploadServiciosCargaMasivaWithApi(accessToken, file);
      setResult(data);
      onSuccess?.(data);
    } catch (err) {
      const msg =
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="servicio-carga-masiva-title"
    >
      <button
        type="button"
        ...
        <div className="flex items-center justify-between border-b border-medical-border bg-medical-primary px-5 py-4">
          <div className="flex min-w-0 items-center gap-2 text-white">
            ...
            <div className="min-w-0">
              <h2 id="servicio-carga-masiva-title" className="text-base font-semibold">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-5 sm:px-6">
          ...
          <div>
            ...
            >
              ...
              <p className="mt-1 text-xs text-medical-mutedText">
                Solo .xlsx · hasta 5 MB · máx. {CARGA_MASIVA_SERVICIOS_MAX_FILAS} filas con datos
              </p>
              {file ? (
          ...
          {result ? (
            ...
            >
              ...
              >
                {result.creados > 0
                  ? `Se crearon ${result.creados} servicio${result.creados === 1 ? "" : "s"} (${result.totalFilas} fila${result.totalFilas === 1 ? "" : "s"} procesada${result.totalFilas === 1 ? "" : "s"}).`
                  : result.totalFilas > 0
                    ? "No se pudo crear ningún servicio."
                    : "No se encontraron filas con datos en la planilla."}
              </p>
              {result.errores.length > 0 ? (
                <>
                  <p className="mt-1 text-xs text-medical-mutedText">
                    Los servicios válidos se guardaron; corregí las filas con error y volvé a subir
                    la planilla.
                  </p>
                  <ul className="mt-2 max-h-40 space-y-1 overflow-y-auto text-xs text-medical-danger">
                    {result.errores.map((item) => (
                      <li key={`${item.fila}-${item.campo ?? ""}-${item.mensaje}`}>
                        ...
                        {formatCargaMasivaServicioError(item)}
                      </li>
                    ))}
                  </ul>
                </>
