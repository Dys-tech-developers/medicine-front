...
import {
  CARGA_MASIVA_PRESTADORES_MAX_FILAS,
  downloadPrestadoresCargaMasivaPlantillaWithApi,
  formatCargaMasivaPrestadorError,
  triggerBrowserFileDownload,
  uploadPrestadoresCargaMasivaWithApi,
  validatePrestadoresCargaMasivaFile,
...
type Props = {
  ...
  onClose: () => void;
  onSuccess?: (result: CargaMasivaPrestadoresResultDto) => void;
};
...
export function PrestadorCargaMasivaDialog({
  open,
  accessToken,
  onClose,
  onSuccess,
  ...
  const [result, setResult] = useState<CargaMasivaPrestadoresResultDto | null>(null);
  ...
  const pickFile = (next: File | null) => {
    ...
    const validationError = validatePrestadoresCargaMasivaFile(next);
    if (validationError) {
  ...
  const handleDownloadPlantilla = async () => {
    ...
    setError("");
    try {
      const { blob, filename } = await downloadPrestadoresCargaMasivaPlantillaWithApi(accessToken);
  ...
  const handleUpload = async () => {
    ...
    setError("");
    try {
      const data = await uploadPrestadoresCargaMasivaWithApi(accessToken, file);
      setResult(data);
      onSuccess?.(data);
    } catch (err) {
      const msg =
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="prestador-carga-masiva-title"
    >
      <button
        type="button"
        ...
        <div className="flex items-center justify-between border-b border-medical-border bg-medical-primary px-5 py-4">
          <div className="flex min-w-0 items-center gap-2 text-white">
            ...
            <div className="min-w-0">
              <h2 id="prestador-carga-masiva-title" className="text-base font-semibold">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-5 sm:px-6">
          <div className="rounded-xl border border-medical-border bg-medical-surface/50 px-4 py-3">
            <p className="text-sm text-medical-text">
              Usá la planilla descargada desde el sistema (hoja «Prestadores»). Los desplegables de
              régimen IVA, estado y servicio habilitado vienen en la hoja oculta «Catalogos». Máximo{" "}
              {CARGA_MASIVA_PRESTADORES_MAX_FILAS} filas y 5 MB por archivo.
            </p>
            <Button
              type="button"
              variant="outline"
          ...
          <div>
            ...
            >
              ...
              <p className="mt-1 text-xs text-medical-mutedText">
                Solo .xlsx · hasta 5 MB · máx. {CARGA_MASIVA_PRESTADORES_MAX_FILAS} filas con datos
              </p>
              {file ? (
          ...
          {result ? (
            ...
            >
              ...
              >
                {result.creados > 0
                  ? `Se crearon ${result.creados} de ${result.totalFilas} fila${result.totalFilas === 1 ? "" : "s"} procesada${result.totalFilas === 1 ? "" : "s"}.`
                  : result.totalFilas > 0
                    ? "No se pudo crear ningún prestador."
                    : "No se encontraron filas con datos en la planilla."}
              </p>
              {result.errores.length > 0 ? (
                <>
                  <p className="mt-1 text-xs text-medical-mutedText">
                    Las filas válidas se guardaron; corregí las siguientes y volvé a subir solo esas
                    filas o la planilla completa.
                  </p>
                  <ul className="mt-2 max-h-40 space-y-1 overflow-y-auto text-xs text-medical-danger">
                    {result.errores.map((item) => (
                      <li key={`${item.fila}-${item.campo ?? ""}-${item.mensaje}`}>
                        ...
                        {formatCargaMasivaPrestadorError(item)}
                      </li>
                    ))}
                  </ul>
                </>
